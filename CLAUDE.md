...
- **crime_points 表是热力图层数据契约**：app.py 不碰 pandas，热力图原始犯罪点从 SQLite `crime_points(lat,lng,severity)` 读（与 edge_risk 同模式）。**采样在 prep 阶段做一次**（`write_crime_points_to_sqlite` 用 `HEATMAP_SAMPLE_N=5000` + 固定 `random_state=0`），表里从此只有 ≤5000 行；`app.py:_load_crime_points` 直接 `SELECT` 全表，不再 `ORDER BY RANDOM() LIMIT`（那是对 10万+ 行全表随机排序，每次冷启动重付）。代价：热力图从"每次渲染换一批随机点"变成"固定同一批点"，视觉密度分布不变，展示场景可接受。
- **`edge_risk(risk_normalized DESC)` 已建索引**（`write_risk_to_sqlite` 里 `CREATE INDEX IF NOT EXISTS idx_edge_risk_risk`），服务 `_load_edge_risk_colors` 的 `ORDER BY risk_normalized DESC LIMIT`——全表排序变索引扫描取前 N 条。`load_risk_weights` 的全表 `SELECT`（无 WHERE/ORDER）本来就要读全部行，索引帮不上。
- **`edge_risk` 新增 `top_crime_category`/`top_crime_count` 列后必须重跑 `python3 prep_data.py`**：`build_edge_risk_table` 的 groupby 顺手多算"该边最常见案件类型 + 次数"（`value_counts().index[0]`/`.iloc[0]`），供路线说明弹窗标出具体案件类型，不走运行时查询。`load_risk_weights` 的 SELECT 显式列出这两列，旧库（重跑前）缺列会让每个 `GET /`/`POST /route` 请求都 `sqlite3.OperationalError` ——与"改 GRAPH_DIST_M 后必须删 walk.graphml"同类的一次性手动步骤，不加 try/except 静默兜底。`avoided_hot_segments` 平行边取值改成"从产出 risk 的同一条边（`max(..., key=risk)`）取 category/count"，不再各字段独立聚合，避免 risk 和 category 报告不一致；`top_crime_count` 不跨平行边求和（120m buffer 会让同一犯罪点被两条平行边重复计数）。`app.py:route()` 把 `avoided + remaining`（互斥，不重复计数）聚合进 `stats.incident_summary`（`Counter` 取 top 3），是 `SAFEPATH_CTX`/`stats` 契约新增字段，模板 `templates/index.html` 的路线说明 `<dialog id="route-explainer">` 读它渲染"Flagged incident types nearby"。
- **Vercel 部署排除 `data/walk.graphml`**（`vercel.json` 的 `functions."app.py".excludeFiles`）：31M 的 graphml 只在 `data/walk.pkl` 缺失/过期时才会被读到（正常路径从不读），随部署包上传纯粹拖慢冷启动解压。`load_graph` 的 fallback 判断 `not os.path.exists(path) or ...`——graphml 在 Vercel 上不存在时安全走 pickle 路径。只影响 Vercel 构建产物，不影响本地 git 仓库 / `prep_data.py` / `flask run`。
...
- **风险上色层用 top-N**（`_RISK_DRAW_LIMIT=20000`，`ORDER BY risk_normalized DESC LIMIT`），不再用固定阈值——百分位排名下固定阈值会失效（`risk>0.4` = 全图 60% 边）。**渲染走单个 `folium.GeoJson`**（`_load_edge_risk_colors` 返回 GeoJSON FeatureCollection，每边一个 LineString feature、颜色存 `properties.color`），不再是 `build_map` 里 for 循环构造 20000 个独立 `folium.PolyLine`——旧方案每个 PolyLine 各自过一遍 Jinja 模板序列化是渲染耗时大头。GeoJSON 坐标顺序 `[lng, lat]`，与 PolyLine 用的 `(lat, lng)` 相反，别搞反。
